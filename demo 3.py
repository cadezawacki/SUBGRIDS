...
# ══════════════════════════════════════════════════════════════════════════════
# RUN 1 — max_charge_delta = 1.5 (tight leash)
# ══════════════════════════════════════════════════════════════════════════════
print("▓" * 110)
print("  RUN 1 — max_charge_delta = 1.5 bps")
print("▓" * 110)

cfg1 = OptimizerConfig(side_floor_pct=0.95, buffer=200.0, max_charge_delta=1.5)
df1, res1 = solve(df, cfg1)
print_report(df1, res1, cfg1)

# ══════════════════════════════════════════════════════════════════════════════
# RUN 2 — max_charge_delta = 0.75 (very tight)
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "▓" * 110)
print("  RUN 2 — max_charge_delta = 0.75 bps")
print("▓" * 110)

cfg2 = OptimizerConfig(side_floor_pct=0.95, buffer=500.0, max_charge_delta=0.75)
df2, res2 = solve(df, cfg2)
print_report(df2, res2, cfg2)

# ══════════════════════════════════════════════════════════════════════════════
# RUN 3 — max_charge_delta = None (uncapped — for comparison)
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "▓" * 110)
...
# ══════════════════════════════════════════════════════════════════════════════
# Comparison: worst-case spread delta across runs
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 110)
print("  COMPARISON: Max |spread delta| per run")
print("=" * 110)
for name, dfr in [("Δ=1.5", df1), ("Δ=0.75", df2), ("Δ=None", df3)]:
    spd_d = dfr.filter(pl.col("quoteType") == "SPD")["spd_delta"].to_numpy()
    skw_d = dfr["skew_delta"].to_numpy()
    print(f"  {name:8s}  max|spd_delta|={np.max(np.abs(spd_d)):.4f} bp   "
          f"max|skew_delta|={np.max(np.abs(skw_d)):.4f}   "
          f"total_proceeds=${dfr['final_proceeds'].sum():,.0f}")
