...
class ConnectionManager:
    ...
    async def _build_payload(self, message: Any, payload_type: Any | BroadcastMessage, *, websocket=None):
        if isinstance(message, payload_type):
            return message
        ...
        if websocket is not None:
            try:
                ...
                message["user"] = user_obj.to_dict() if hasattr(user_obj, "to_dict") else dict(user_obj)
            except Exception:
                message["user"] = message.get("user", {"username": "error"})
        else:
    ...
    async def _handle_upload(self, websocket: WebSocket, d: Dict[str, Any]):
        ...
        # pku = portfolio_key.upper()
        # pt_ctx = RoomContext(
        #     room=f"{pku}.PORTFOLIO",
        #     grid_id="portfolio",
        #     grid_filters={"field": "portfolioKey", "op": "eq", "value": portfolio_key},
        #     primary_keys=["portfolioKey", "tnum"],
        # )
        # meta_ctx = RoomContext(
        #     room=f"{pku}.META",
        #     grid_id="meta",
        #     grid_filters={"field": "date", "op": "eq", "value": date_str},
        #     primary_keys=["portfolioKey", "date"],
        # )
        #
        # await self._send_direct_message(
        #     websocket,
        ...
        #         toastType="info",
        #         toastTitle="Upload",
        #         toastMessage="Persisting + broadcasting…",
        #         toastId=toast_id,
        #         toastOptions=TOAST_PERSIST_OPTS,
        #     ),
        # )
        #
        # opts = PayloadOptions(persist=True, broadcast=True, trigger_rules=True, relay=True)
        # await self._forward_to_grid(Publish(add=my_pt, pk_columns=["portfolioKey", "tnum"], context=pt_ctx, options=opts, trace=trace))
        # await self._forward_to_grid(Publish(add=meta_df, pk_columns=["portfolioKey", "date"], context=meta_ctx, options=opts, trace=trace))
        # await publish_to_ptinternaldata(portfolioKey=portfolio_key, event="New Portfolio")
    ...
    async def _send_direct_message(self, websocket: WebSocket, payload: Union[Message, Dict, bytes, bytearray, memoryview]):
    ...
    # =========================================================================
    # Arrow IPC Reactive Subscription Handlers
    # =========================================================================
    #
    # These handlers integrate the Arrow IPC query engine's reactive
    # subscription system into the existing ConnectionManager dispatch table.
    #
    # Arrow client IDs are generated per WebSocket and stored in a mapping
    # so cleanup can happen alongside the existing socket cleanup path.
    # =========================================================================

    # ── Token → Arrow client ID mapping ──────────────────────────────────────
