...
export class PortfolioPage extends PageBase {
    ...
    setupEmitter() {
        const adapter = this;
        const te = this.emitter.on('cell-edit', async (payloads) => {
            const sm = adapter.subscriptionManager();
            ...
            await this.subscriptionManager().publishMessage(room, meta, data);
        });
        this.emit_tokens.set(te, 'cell-edit');
    ...
    setupStores() {
        ...
        // this._subs.push(this.portfolioMeta$.onValueAddedOrChanged('rfqSide', ()=>{
...
-- Total in comp: Multi Dealer`
    ...
    _readWeightCache() {
        try {
            ...
            return (v === 'dv01' || v === 'count') ? v : 'notional';
    ...
    async onCleanup() {
        ...
        if (this.progressBarPct) {
            try { this.progressBarPct.numberFlowElement.remove(); } catch(_){}
            try { this.progressBarPct.destroy(); } catch(_){}
        }
        if (this.progressBarCount) {
            try { this.progressBarCount.numberFlowElement.remove(); } catch(_){}
    ...
    _setupDropdown() {
        if (this.stateDropdown) {
            ...
            ["LIVE", 'WON', 'COVERED', 'MISSED', 'DNT', 'PASSED', 'NETDOWN', 'INDICATIVE', 'ERROR', 'CANCELLED', "EXPIRED", 'NEW', 'PENDING', 'TEST', "TRANSFER", 'OTHER', 'UNKNOWN'].forEach(stateValue => {
                const option = this._renderStateEditorOption(stateValue);
    ...
    async _initializeManagersAndWidgets() {
        ...
        this.refDropdown = new ConfigurableDropdown(this.context, '#my-dropdown-container', '#refmkt-btn', {
            ...
            checkDataAvailability: async (selected, state, metricsData, adapter) => {
                ...
                const mkts = engine.getColumnValues('refMktMkt');
                ...
                const fields = new Set(adapter.getAllFields());
            ...
            onChange: (newState) => {
                ...
                // Ensure market data columns are loaded before the UI reacts.
                // For 'Dynamic' mode ensure ALL market columns; otherwise just the selected market.
                ...
                this.refDropdownLower.setValue('market', mkt, false, false);
                this.refDropdownLower.setValue('side', newState.selected.side, false, false);
        ...
        this.refDropdownLower = new ConfigurableDropdown(this.context, '#my-dropdown-container-lower', '#refmkt-btn-lower', {
            ...
            checkDataAvailability: async (selected, state, metricsData, adapter) => {
                ...
                const mkt = state.selected.market;
                const marketsToLoad = mkt === 'Dynamic'
                    ? marketDataArr
                    : [mkt];
                this._ensureMarketColumns(marketsToLoad).catch(() => {});
            ...
            onChange: (newState) => {
                ...
                this.refDropdown.setValue('market', mkt, false, false);
                this.refDropdown.setValue('side', newState.selected.side, false, false);
    ...
    async _handleMetaSubscription(message) {
        ...
        this._subs.push(this._metaStore.pick(['client', 'state']).onChanges(()=>{
    ...
    setupMetaEvents() {
        ...
        // this.portfolioMeta$.onValueAddedOrChanged('pctPriced', async (pct)=>{
        //     if (pct == null) return;
        //     pct = Number(pct);
        //     if (isNaN(pct)) {
        //         // await this._setProgressBar(0, 0, 0);
        //         console.error("Invalid progress:", pct)
        //         return;
        //     }
        //     const total = Number(this.portfolioMeta$.get('count'));
        //     if (this.page$.get('domAnimationFinished')) {
        //         await this._setProgressBar(pct*100, (pct*total).toFixed(0), total);
        //     }
        // });
    ...
    _teardownGrid() {
        ...
        if (this.emit_tokens.size) {
            this.emit_tokens.forEach((eventName, token) => {
                try { this.emitter.off(eventName, token); } catch (_) {}
            });
            this.emit_tokens.clear();
        }
    ...
    async _handlePortfolioSubscription(message) {
        ...
        for (const x of this.marketDataMap.keys()) {
            n.push(...generateAllMarketColumns(x));
        }
        ...
        // this.engine.setRequiredCols([
        //     'newLevel', 'newLevelDisplay', 'refLevel', 'refPx', 'refSpd', 'assignedTrader', 'desigTrader', 'refSkew',
        //     'isInDefault', 'daysToSettle', 'isRfqBenchmarkMismatch', 'desigName', 'isBvalBenchmarkMismatch', 'isMacpBenchmarkMismatch', 'grossSize', 'grossDv01',
        //     'isDnt', 'restrictedCode', 'firmAggPosition', 'isNewIssue', 'isMuni', 'description', 'currency'
        // ])
    ...
    async _parsePublishedPortfolioData(message) {
        ...
        if (_apply) {
            const r = await this.ptGrid.applyServerTransaction(payloads, {emitAsEdit: false});
        }
    ...
    setupUser() {
        ...
        this._subs.push(this.userManager().userProfile$.selectKeys(['displayName','username']).onChanges( (ch) => {
            grid.api.redrawRows()
        }));
    }
    ...
    setupGridReactives() {
        ...
        this.emit_tokens.set(filterToken, ArrowAgGridAdapter.FILTER_EVENT);
        ...
        const rowCountToken = this.emitter.on(pt_row_event, async () => {
            if (this.progressBarCount) this.progressBarCount.setConfig({'suffix': `/${this.engine.numRows()}`});
            await this.updatePctPriced()
        });
        this.emit_tokens.set(rowCountToken, pt_row_event);
    ...
    setupSkewReactions() {
        ...
        this._subs.push(this.overallSkew$.onChanges(debounce(() => {
            requestIdleCallback(()=>{
                this.updateSkew();
            },{timeout:100})
        }, 100)))
    ...
    updateSkew() {
        ...
        if (this?.bidSkew) {
            const v = roundToNumeric(skews.bid?.value, 2);
    ...
    setupGridEpochReactions() {
        this._subs.push(this.engine.onDerivedDirty(debounce(async (colsChanged) => {
            if (colsChanged?.has('isPriced')) {
                await this.updatePctPriced();
            }
        }, 250)));
    }

    async _getCountOfPriced(){
        return (await this.engine.getColumnValues('isPriced')).reduce((accumulator, currentValue) => accumulator + currentValue, 0) || 0;
    }

    async updatePctPriced(pctPriced = null) {
        const total = this.engine.numRows();
        if (!total) return 0;
        if (pctPriced == null) {
            const priced = (await this._getCountOfPriced()) ?? 0;
            pctPriced = total > 0 ? (priced / total) : 0;
        }

        pctPriced = Number(pctPriced);
        if ((pctPriced == null) || isNaN(pctPriced)) {
            console.error("Invalid progress:", pctPriced)
            return;
        }
        await this._setProgressBar(pctPriced*100, (pctPriced*total).toFixed(0), total);
    }
