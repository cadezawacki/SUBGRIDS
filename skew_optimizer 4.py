...
# BSI > BSR because adding risk (BSI) requires more compensation than
# shedding risk (BSR).  Both decline with liquidity.
# ══════════════════════════════════════════════════════════════════════════════
DEFAULT_BSR_BSI_MATRIX: dict[int, tuple[float, float]] = {
    1: (4.00, 5.00), 2: (3.50, 4.50), 3: (3.00, 4.00), 4: (2.50, 3.50),
    5: (2.00, 3.00), 6: (1.50, 2.50), 7: (1.00, 2.00), 8: (0.75, 1.50),
    9: (0.50, 1.00), 10: (0.25, 0.50),
}
...
class OptimizerConfig:
    ...
    # ── max per-bond charge movement (in charge-bps) ──
    # For SPD bonds 1 charge-bp = 1 spread-bp.
    # For PX bonds 1 charge-bp = 0.01 price points.
    # Set to None to disable.
    max_charge_delta: float | None = 1.5

    # If True, auto-relax max_charge_delta on infeasibility (2×, 4×, uncapped).
    # If False, report infeasibility and return starting values.
...
class OptimizationResult:
    ...
    requested_max_delta: float | None = None  # what was asked for
    actual_max_delta: float | None = None      # what was actually used (after relaxation)
    fell_back: bool = False                     # True if auto-relaxed
...
def print_report(df_result: pl.DataFrame, result: OptimizationResult, cfg: OptimizerConfig | None = None) -> None:
    ...
    fb_note = ""
    if result.fell_back:
        fb_note = f"  (relaxed from {result.requested_max_delta} → {result.actual_max_delta})"
    print("=" * 110)
    print(f"  CHARGE OPTIMIZATION REPORT")
    print(f"  status        : {result.status}")
    print(f"  objective     : ${result.objective_value:,.0f}")
    print(f"  max_charge_Δ  : requested={result.requested_max_delta}  actual={result.actual_max_delta}{fb_note}")
    print("=" * 110)
