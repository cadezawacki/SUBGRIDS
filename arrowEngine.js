...
const FREEZE_MS = 450;         // how long to guard after a preset/large change
const RAF_REAPPLY_COUNT = 2;   // number of RAF ticks to re-assert widths
...
function _tryConcatArrowTables(left, right) {
    ...
    // Fallback: rebuild once without JS object materialization
    ...
    for (let i = 0; i < fields.length; i++) {
        ...
        for (let r = 0; r < ln; r++) b.append(_coerceArrowScalar(lv.get(r)));

        const rn = right.numRows >>> 0;
        for (let r = 0; r < rn; r++) b.append(_coerceArrowScalar(rv.get(r)));
...
function ensureOverlay(elementOrSelector) {
    ...
    if (overlayMap.size > 16) {
        for (const [k, el] of overlayMap) {
            if (!document.body.contains(el)) { try { el.remove(); } catch {} overlayMap.delete(k); }
        }
    }
...
function flashSelection(elementOrSelector=null, api=null) {
    switch (FLASH_MODE) {
...
function getConsecutiveFromMap(mp) {
    if (mp.size === 0) {return []}
    const results = [];
    const arr = Array.from(mp.entries())

    let currentSet = [arr[0][0]];
    let currentCols = arr[0][1];
    for (let i = 1; i < arr.length; i++) {
        const isCon = arr[i][0] === (arr[i - 1][0] + 1);
        const isMatch = currentCols.size === arr[i][1].size && [...currentCols].every(c => arr[i][1].has(c))
        if (isCon && isMatch) {
            currentSet.push(arr[i][0]);
        } else {
            results.push([currentSet, currentCols]);
            currentSet = [arr[i][0]];
            currentCols = arr[i][1];
        }
    }
...
function diagonalJoin(...arrays) {
    const maxLength = Math.max(...arrays.map(arr => arr.length));
    return Array.from({ length: maxLength }, (_, index) => {
        return arrays.map(arr => arr[index] ?? null);
    });
}

function diagonalJoinObj(obj1, obj2) {
    if (!Object.keys(obj1).length) return obj2
    if (!Object.keys(obj2).length) return obj1
    ...
    for (let i = 0; i < maxLength; i++) {
        const entry1 = entries1[i] || [null, null];
        const entry2 = entries2[i] || [null, null];
        const joinedObject = {
            key1: entry1[0] ?? null,
            value1: entry1[1] ?? null,
            key2: entry2[0] ?? null,
            value2: entry2[1] ?? null,
        };
...
class EditSignalCoalescer {
    ...
    _runSlice() {
        ...
        for (let i = 0; i < promises.length; i++) {
            ...
            pr.finally(() => this._inFlight.delete(pr));
        }
...
//     // Fast projection by name: keep columns that exist; fill missing with nulls.
//     const srcFields = sourceTable.schema.fields;
//     const srcMap = new Map();
//     for (let i = 0; i < srcFields.length; i++) srcMap.set(srcFields[i].name, sourceTable.getChildAt(i));
//
//     const outCols = {};
//     const n = sourceTable.numRows | 0;
//
//     for (let i = 0; i < targetSchema.fields.length; i++) {
//         const f = targetSchema.fields[i];
//         const vec = srcMap.get(f.name);
//         const isBig = (f.type instanceof arrow.Int64) || ((f.type instanceof arrow.Int) && f.type.bitWidth === 64);
//         let bt = f.type;
//         if (isBig) bt = new arrow.Float64();
//         const b = arrow.makeBuilder({ type: bt, nullValues: [null, undefined] });
//
//         if (vec) {
//             for (let i of vec.toArray()) {
//                 if (isBig || (typeof i === 'bigint')) {
//                     b.append(Number(i));
//                 } else {
//                     b.append(i)
//                 }
...
//             }
//         } else {
//             for (let r = 0; r < n; r++) b.append(null);
//         }
//
//         outCols[f.name] = b.finish().toVector();
//     }
...
export class ArrowEngine {
    ...
    _emitTableWillReplace(prevTable, nextTable){
        try {
            ...
            this._em.emit('table-will-replace', { prevNumRows, nextNumRows, prevNumCols, nextNumCols });
        } catch {}
    }
    ...
    _getVector(name){
        if (!this._nameToVector.has(name)) {
            ...
            if (idx == null) throw new Error('Column not found: '+name);
    ...
    // getRowObject(rowIndex, columns=null, raiseOnMissing=false){
    //     const names = this._normalizeColumnSelector(columns);
    //     const obj = { [this._idProperty]: this.getRowIdByIndex(rowIndex), [ this._idxProperty ]: rowIndex};
    //     for (let i=0;i<names.length;i++) {
    //         try {
    //             obj[names[i]] = this.getCell(rowIndex, names[i]);
    //         } catch (e) {
    //             console.log('Error in getRowObject:', e)
    //             if (raiseOnMissing) throw Error(e)
    //         }
    //     }
    //     return obj;
    // }
    ...
    _ensureOverlay(name){
        try {
            ...
            ov = overlayKind === 'number'
                ? { kind: 'number', mask: new Uint8Array(n), values: new Float64Array(n), size: 0 }
                : { kind: 'other', map: new Map(), size: 0 };
    ...
    _overlayHas(ov, ri){
        ...
        if (ov.kind === 'number'){
            ...
            if (ri < 0 || ri >= mask.length) return false;
    ...
    _overlayGet(ov, ri){
        ...
        if (ov.kind === 'number'){
            const m = ov.mask ? ov.mask[ri] : MASK_NONE;
            if (m === MASK_NULL) return null;
    ...
    setOverlayValuesByColumnBatch(column, rowIndices, values, { freeze = false, bump = true } = {}) {
        ...
        // Prefer zero-allocation path when caller already provides Int32Array.
        ...
        let rowsOut = null;
        let outCount = 0;
        let copying = !rowsIsI32; // non-I32 always uses rowsOut for safety

        if (copying) rowsOut = new Int32Array(n0);

        // Reusable result object to avoid per-cell allocation in the hot loop.
        const _cnfResult = { kind: 0, num: 0 };
        const coerceNumberFast = (v) => {
            // mutates and returns _cnfResult: kind 0=unset, 1=null, 2=value
            ...
            // Only pay regex when we see separators.
        ...
        const rowsChanged = copying ? ((outCount && rowsOut) ? rowsOut.subarray(0, outCount) : new Int32Array(0))
    ...
    // getDependenciesClosure(name){
    //     if (this._depClosureCache.has(name)) return Array.from(this._depClosureCache.get(name));
    //     const out = new Set();
    //     const seen = new Set();
    //     const dfs = (n)=>{
    //         if (seen.has(n)) return;
    //         seen.add(n);
    //         const direct = this._columnDependencies.get(n);
    //         if (!direct || direct.size===0) return;
    //         for (const d of direct){
    //             if (d === n) continue;
    //             out.add(d);
    //             if (this._derived.has(d)) dfs(d);
    //         }
    //     };
    //     dfs(name);
    //     this._depClosureCache.set(name, out);
    //     return Array.from(out);
    // }
    //
    // getDependentsClosure(cols){
    //     const inputs = Array.isArray(cols) ? cols : [cols];
    //     const out = new Set(inputs);
    //     const stack = [...inputs];
    //     while (stack.length){
    //         const cur = stack.pop();
    //         let cached = this._dependentsClosureCache.get(cur);
    //         if (!cached){
    //             cached = new Set();
    //             const direct = this._columnDependents.get(cur);
    //             if (direct && direct.size){
    //                 const localVisited = new Set();
    //                 const dfsStack = [...direct];
    //                 while (dfsStack.length){
    //                     const n = dfsStack.pop();
    //                     if (localVisited.has(n)) continue;
    //                     localVisited.add(n); cached.add(n);
    //                     const nxt = this._columnDependents.get(n);
    //                     if (nxt && nxt.size) dfsStack.push(...nxt);
    //                 }
    //             }
    //             this._dependentsClosureCache.set(cur, cached);
    //         }
    //         for (const d of cached){ if (!out.has(d)){ out.add(d); stack.push(d); } }
    //     }
    //     return Array.from(out);
    // }
    ...
    async materializeEdits({ commit = 'auto' } = {}) {
        ...
        return new Promise((resolve) => {
            ...
            this._materializeTimer = setTimeout(async () => {
                ...
                try {
                    const result = await this._materializeEdits({ commit });
                    for (let i = 0; i < resolvers.length; i++) resolvers[i](result);
                } catch (e) {
                    // Ensure all waiters resolve even on failure to prevent dangling promises
                    for (let i = 0; i < resolvers.length; i++) resolvers[i](this.table);
                }
            }, 200);
        });
    }
    ...
    _materializeEditsInner(commit, scopeCols) {
        ...
        for (let i = 0; i < physNames.length; i++) {
            ...
            if (ov.kind === 'number') {
                ...
                for (let r = 0; r < n; r++) {
                    const m = mask[r] | 0;
                    if (m === 0) { builder.append(vec.get(r)); continue; }
                    if (m === 1) { builder.append(null); continue; }
                    const v = vals[r];
    ...
    // replaceAllRows(rowsOrTable, {preserveSchema=true, resetOverlays=true, rebuildIndexes=true, emitGlobalEpoch=false, useMasterSchema=true} = {}) {
    ...
    //         if (preserveSchema) {
    //             const cur = this.table.schema.fields.map(f => f.name);
    //             const nxt = rowsOrTable.schema.fields.map(f => f.name);
    //             const sameOrder = cur.length === nxt.length && cur.every((n, i) => n === nxt[i]);
    //             const targetSchema = useMasterSchema ? this.master_schema : this.table.schema;
    //             // nextTable = sameOrder ? rowsOrTable : projectTableToSchema(rowsOrTable, targetSchema);
    ...
    /* --------------------------- row window helpers -------------------------- */
    getAllRows({format='objects', columns=null}={}){
        const s = 0; const e = this.numRows();
    ...
    addRows(rows, updateIfExists = true) {
        ...
        if (updateIfExists) {
            for (let i = 0; i < list.length; i++) {
                ...
                if (existingIdx >= 0) updateRows.push({ rowIndex: existingIdx, row });
    ...
    removeRowsByIndices(rowIndices) {
        ...
        for (let i = 0; i < names.length; i++) {
            const name = names[i];
            const vec = this._getVector(name);
            const out = new Array(n0 - remIdx.length);
            let dst = 0;
            for (let src = 0; src < n0; src++) {
                if (removedSet.has(src)) continue;
                let v = vec.get(src);
...
export class ArrowAgGridAdapter {
    ...
    constructor(context, engine, columnDefs, opts = {}, {name='portfolio', globalViews=[], enableFilterMemory=true, enablePasteOverride=false}={}) {
        ...
        this.getRows = debouncePerArgs(this._getRows, 100, {leading:true, trailing:false});
    ...
    _notifyFirstEvent(...args) {
        ...
        this._em.emitAsync(ArrowAgGridAdapter.FIRST_EVENT, ...args);
    };
    ...
    mount(elementOrSelector) {
        ...
        const baseGridOptions = Object.assign({
            ...
            onColumnVisible: async (e) => {
                try { this._processColumnVisibleEvent(e) } catch {}
            },
            rowClassRules: {
                'error-row': (params) => {
                    if (!engine.columnDefs.has('desigName')) return
                    const ri = params?.data?.[this._idxProperty];
                    if (ri == null) return;
                    const desig = params.data?.desigName ?? engine.getCell(params.data?.[this._idxProperty], 'desigName');
                    if (desig == null) return true;
                    const assigned = params.data?.assignedTrader ?? engine.getCell(params.data?.[this._idxProperty], 'assignedTrader');
                    if (assigned == null) return true;
                    const description = params.data?.description ?? engine.getCell(params.data?.[this._idxProperty], 'description');
                    if (description == null) return true;
                    const ref = params.data?.refLevel ?? engine.getCell(params.data?.[this._idxProperty], 'refLevel');
                    if (ref == null) return true;
                    return null;
                },
                // 'typo-flag': (params) => {
                //     if (!engine.columnDefs.has('newLevelDisplay') || !engine.columnDefs.has('refLevel')) return
                //     const ri = params?.data?.__srcIndex;
                //     if (ri == null) return;
                //     let nl = params.data?.newLevelDisplay ?? engine.getCell(params.data?.__srcIndex, 'newLevelDisplay');
                //     let rl = params.data?.refLevel ?? engine.getCell(params.data?.__srcIndex, 'refLevel');
                //     nl = coerceToNumber(nl, {onNaN:null});
                //     rl = coerceToNumber(rl, {onNaN:null});
                //     if (nl == null) return true;
                //     if (!rl) return;
                //     if ( ((nl / rl) > 2) || (nl / rl < 0.5)) return true
                //     return null;
                // },
                'trader-name-matches': (params) => {
                    if (!engine.columnDefs.has('desigTraderId')) return
                    const ri = params?.data?.[this._idxProperty];
                    if (ri == null) return;
                    if (this.pending_styles.has(ri)) {
                        return coerceToNumber(this.pending_styles.get(ri)['claimed']) === 1
                    }
                    ...
                    const desig = params.data?.desigTraderId ?? engine.getCell(params.data?.[this._idxProperty], 'desigTraderId');
                    const assigned = params.data?.assignedTrader ?? engine.getCell(params.data?.[this._idxProperty], 'assignedTrader');
                ...
                'removed-row': (params) => {
                    if (!engine.columnDefs.has('isReal')) return
                    const ri = params?.data?.[this._idxProperty];
                    if (ri == null) return;
                    if (this.pending_styles.has(ri)) {
                        return coerceToNumber(this.pending_styles.get(ri)['isReal']) === 0
                    }
                    const real = params.data?.isReal ?? engine.getCell(ri, 'isReal');
    ...
    /**
     * Toggle the paste-override modal on or off at runtime.
     * If the manager hasn't been created yet, it will be lazily initialized.
     * @param {boolean} flag
     */
    ...
    async loadGridState(state) {
        ...
        // 0) Temporarily disable autosize to prevent post-apply shrink. (You currently set fitCellContents.) :contentReference[oaicite:2]{index=2}
        ...
        try {
            ...
            // 1) Apply column ordering/pinning/visibility/sort in one shot.
            if (state.columnState && state.columnState.length) {
                // Apply exactly as provided. Order + pins honored here.
                this.api.applyColumnState({
                    state: state.columnState,
                    applyOrder: true,
                    // defaultState omitted here because your Tree loader already purges stale flags
                });
            }

            // 2) Apply models AFTER column state to avoid churn.
            ...
            // 3) Reinforce widths explicitly in a single batch. This beats autosize and any late events.
            const widths = [];
            ...
            // 4) Light refresh only; avoid purge which can jolt sizes. Your old code purged here. :contentReference[oaicite:3]{index=3}
            try { this.api.refreshHeader(); } catch {}
            try { this.api.refreshServerSide({ purge: false }); } catch {}

            // Inform UI that preset load completed (prevents header flash spam elsewhere).
            try { this.grid$.set('presetLoaded', true); } catch {}

        } finally {
            // 5) Once a preset with explicit widths has been loaded, permanently
            //    disable the autoSizeStrategy (e.g. fitCellContents) so that
            //    subsequent data refreshes / epoch changes do not shrink columns
            //    back to min-width.  Only restore the previous strategy when the
            //    loaded state carried NO width information at all.
            const hasExplicitWidths = Array.isArray(state.columnState) &&
                state.columnState.some(c => typeof c.width === 'number' && c.width > 0);
            if (hasExplicitWidths) {
                // Keep autosize disabled — preset widths should stick.
                try { this.api.setGridOption('autoSizeStrategy', undefined); } catch {}
                try { this.api.setGridOption('suppressAutoSize', true); } catch {}
            } else {
                // No widths in preset — restore original autosize behaviour.
    ...
    addColumns(colDefs = [], { show = false } = {}) {
        ...
        if (show) {
            this.setColumnsVisible(colDefs.map(d => d.colId || d.field), true);
        }
    }
    ...
    _buildAgColumnDefs(userDefs) {
        ...
        const normalize = (d) => {
            ...
            // if (!out.icons.filter) out.icons.filter = `<div class="ag-filter-lottie-wrapper"><lord-icon src="/assets/lottie/filter.json" trigger="in" class="ag-filter-lottie-icon current-color"></lord-icon><span>${filterName}</span></div>`
    ...
    buildPredicate(col, model) {
        ...
        if (ft === 'set' || ft === 'agsetcolumnfilter') {
            ...
            // console.log(`filtering ${col} for...`, Array.from(vals))
            if (!vals) return null;
            return (ri) => {
    ...
    evaluateFilterModel(filterModel) {
        ...
        const fullIndex = (n) => { const a = new Int32Array(n); for (let i = 0; i < n; i++) a[i] = i; return a; };
        if (!keys.length || n===0) return fullIndex(n);
    ...
    async applyServerTransaction(payloads, opts = {}) {
        ...
        const SANITIZE_FINITE = false;   // set true to also coerce ±Infinity → null
    ...
    async applyServerUpdateTransaction(updates, opts = {}) {
        ...
        const {
            includeAllColumns = false,
            commit = false,               // 'auto'|true|false — materialize overlays after apply
        ...
        // First pass: validate rows and collect per-row payloads (dedupe multiple payloads for same rowIndex)
        const rowWorkMap = new Map(); // rowIndex -> array of payload objects to fold
        for (let i = 0; i < list.length; i++) {
        ...
        // Dispatch a single server-side transaction to AG Grid
        if (rowsForAg.length) {
        ...
        try {
            ...
            // if (emitAsEdit) {
            //     engine.emitter.emit('batch-edit', {
            //         rowsChanged: rowsArr,
            //         colsChanged: colsArr,
            //         patches: editPatches,
            //         engine: this.engine
            ...
            // Fallback: mark rows for repaint if epoch emit fails for any reason
        ...
        // If any of the changed columns are flagged to trigger a redraw, do so in one pass
        if (checkRedraw !== false) {
            try {
                for (let i = 0; i < colsArr.length; i++) {
                    const fld = colsArr[i];
                    if (this.columnRegistry.getColumnDef(fld)?.context?.triggerRedraw) { redrawFlag = true; break; }
                }
                if (redrawFlag) {
                    const nodes = [];
        ...
        // If active filters touch changed columns, refresh SSRM once (no purge)
        try {
            const fm = api.getFilterModel?.() || {};
            const filteredCols = Object.keys(fm);
            let hit = false;
            for (let i = 0; i < filteredCols.length; i++) {
                if (changedCols.has(filteredCols[i])) { hit = true; break; }
            }
