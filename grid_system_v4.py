...
class SnapshotLeases:
    ...
    async def ensure(self, context: RoomContext) -> int:
        # M-19: key by full context (grid_id + filters) to disambiguate actors
...
class GridSystem:
    ...
    async def ingest_micro_publish(
        ...
        # 1. Apply edit
        await log.micro('starting apply edit')
        delta_df = await actor.apply_edit(payloads, user=user)
        await log.micro('ending apply edit')

        if (delta_df is None) or delta_df.hyper.is_empty():
