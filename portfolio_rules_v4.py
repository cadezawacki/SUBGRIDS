...
async def risk_adjust(ctx):
    ...
    data = (await ctx.running_delta_slice(['tnum', 'portfolioKey'] + ['unitDv01','unitAccrued','unitCs01','unitCs01Pct','grossSize','axeFullBidSize','axeFullAskSize','netFirmPosition','netAlgoPosition','netStrategyPosition','netDeskPosition','signalFlag','side']))
...
async def edit_audit(ctx):
    ...
    try:
        ...
        delta = await ctx.running_delta_slice(['newLevel', 'lastAdminEditNewLevel', 'lastTraderEditNewLevel'])
        ...
        cols = pk_cols + ['newLevel']

        if (name is not None) and (str(name).upper() != "SERVER"):
            ts = isonow(True)
            exprs.extend([
                pl.lit(name).alias("lastEditUser"),
                pl.lit(ts).alias("lastEditTime"),
            ])
            if username in PACT_USERNAMES:
                exprs.extend([
                    pl.lit(name).alias("lastAdminEditUser"),
                ...
                exprs.extend([
                    pl.lit(name).alias("lastTraderEditUser"),
            ...
            if exprs:
                return delta.select(cols).with_columns(exprs).drop(['newLevel'], strict=False)
    except Exception as e:
